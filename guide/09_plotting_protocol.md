...
候选阶段只保留可编辑源文件（包括脚本、Draw.io 文件或 Penpot 原生 Board）和 PNG 预览；模板评审、证据、数据快照、比较与检查记录照常保存。将候选 PNG 交给用户，**用户明确确认最终版本后**，才为选定版本生成并保留 PDF、SVG 等正式格式；未入选候选仍只保留源文件和 PNG。若生成 PNG 必须经过 PDF 等中间格式，应在临时构建目录转换，不将中间文件留在单图交付目录。最终矢量质量检查在正式导出后进行。
