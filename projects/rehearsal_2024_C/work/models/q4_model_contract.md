...
3. 类别 schema 固定为四材料、三波形、四温度：真正不在该 schema 的外部值在入口校验时报错；留一材料/温度压力测试仍使用预声明 schema，训练折未出现但 schema 已知的水平由 `OneHotEncoder(handle_unknown="ignore")` 映射为该字段全零，必须单独标为“已知水平外推”。
4. 预测必须有限且严格大于 0。
...
| 类别编码与 schema | 预声明四材料、三波形、四温度；`OneHotEncoder(handle_unknown="ignore")` 仅服务于已知水平留一压力测试；schema 外值先行报错 |

主候选最多 18 个固定抽样组合，挑战者最多 8 个；`random_state=20240920`，线程数不超过物理核数减一，单作业硬上限 120 分钟。完整网格和固定抽样上限同时登记在 `experiments/s2_frozen_config.json`。
...
- 主指标：外层 OOF RMSLE，按共享合同的自然对数 `log1p` 公式计算；
- 辅助：MAE、RMSE、MAPE、中位绝对百分比误差、原尺度 $R^2$；MAPE 分母保护和 $R^2$ 尺度以共享合同为准；
- 子组：四材料、三波形、四温度、固定 $f/B_m$ 分箱；升级门槛中的“主要子组”须有 `n≥100` 个 OOF 样本且覆盖至少 3 个实际外层折，否则只作描述性报告；
- 泛化压力：留一材料、留一温度、边界频率和高损耗尾部；留一测试中的 schema 已知未见水平与真正 schema 外输入必须分开记录；
...
树模型替代 Ridge 的最低条件：RMSLE 相对降低至少 2%，且每个满足 `n≥100` 且覆盖至少 3 个外层折的主要子组 RMSLE 不恶化超过 10%；未达到有效组标准的子组仍公开样本量和误差，但不用于事后改变升级门槛。差异不稳定时选 Ridge。
...
- 任一预先有效的主要子组（`n≥100` 且覆盖至少 3 折）恶化超过 10%，或误差集中在某材料无法解释；
- 留一材料/温度性能崩溃却声称普遍适用；
