...
| 2026-09-09 | X0019 | S4 | G3 授权候选的正式 S4 运行 | clean commit 3295667；13 个训练 CSV；15 primary outer folds、13 LOSO；Q2 4 配置/条件权重；Q3 8 候选/1 次 AP-count 对照；bootstrap=1,000 | `conda run -n math_modeling python projects/rehearsal_2024_B/src/s4_main.py`；实际 nested Q1 血缘、逐行 gzip、promotion 与唯一全量配置 | PASS；1,250 行/482 组、1,751 fits、0 warnings、448 lineage 且 overlaps=0；Q2 weighted gain=+0.019002 未晋级；Q3 unified S=0.550330 晋级；test numeric read/prediction=0/0 | PASS |
| 2026-09-09 | X0020 | S4 | S4 独立复算与 G4 提交前边界检查 | results/raw/main JSON/gzip、run/validation manifest、正式源码、Git 索引和本地 17 CSV | `s4_validation.py` 正式与 `--check-only`；py_compile；真实入口 `--verify-only`；CSV ignore/track、verified/review、gzip、source drift、diff 检查 | 74/74 checks PASS；verify-only 为 0 fits/0 reads；17 CSV ignored/0 tracked；verified=0；gzip JSONL=3/plain=0；无 gate_4_review；正式源码漂移=0；diff-check PASS | PASS |

| 2026-09-11 | X0021 | S5 | G4 PASS 后的独立结果重算与 pre-release freeze | q2/q3 selection traces、S3/S4 OOF、Baseline metrics、冻结配置、G4 已核验产物哈希；测试仅字节身份 | `s5_validation.py --phase prepare-freeze`；手工实现 fixed-17 F1、nearest-rank ARE90、system grouping、词典序与 promotion，不导入生产 evaluate 函数 | PASS；selection/promotion 6/6；freeze 完整；Q3 管线/配置 Evidence ID 分离；official numeric read=0 | PASS |
| 2026-09-11 | X0022 | S5 | exactly-once full-data fit 与官方测试最终释放 | clean source 474acb7、freeze A4AA45A0、disk ledger prior success=0、13 train + 4 frozen test identities | `s5_release.py --preflight-only` 后运行 `s5_release.py`；全量 Q1/Q2/Q3 冻结栈，staging 验证后原子发布并写 ledger | PASS；release ID S5-A4AA45A0491C-20260911T110151+0800；29.320 s、4 fits、0 warnings；13/4 numeric reads；四类预测输出；feedback=false | PASS |
